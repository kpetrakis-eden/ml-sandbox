...
SRC_DATA_ROOT = DRIVE / f"data/raw/{DATASET_NAME}"
DST_DATA_ROOT = DRIVE / f"data/processed/{DATASET_NAME}"
...
for cls_dir in tqdm(SRC_DATA_ROOT.iterdir()):
  ...
  for img_path in train_images:
    shutil.copy2(img_path, train_class_folder / img_path.name)
  for img_path in dev_images:
    shutil.copy2(img_path, dev_class_folder / img_path.name)